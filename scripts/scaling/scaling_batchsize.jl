...
function init_parameters(rng, p_true, perturb = 1e0)
    distrib_param = NamedTuple([dp => Product([Uniform(sort([(1e0 - perturb / 2e0) * k,
                                                   (1e0 + perturb / 2e0) * k])...)
                                               for k in p_true[dp]]) for dp in keys(p_true)])

    p_transform = Bijectors.NamedTransform(NamedTuple([dp => bijector(distrib_param[dp])
...
for batchsize in batchsizes, infer_ics in (true, false)
    ...
    push!(results, (; times = stats.times, batchsize, memory = stats.memory, infer_ics))
end
