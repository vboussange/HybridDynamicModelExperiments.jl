...
we initialise the parameter in the vicinity of the true parameters
=#
cd(@__DIR__)
...
u0_true = [0.8,0.8,0.8]
name_scenario = "naive_inference_neighbourhood"
...
em_init = EcosystemModelMcKann(x_c = em_true.p[1] + 0.01 * (em_true.p[1]), 
                                x_p = em_true.p[2] + 0.01 * (em_true.p[2]), 
                                y_c = em_true.p[3] + 0.01 * (em_true.p[3]), 
                                y_p = em_true.p[4] + 0.01 * (em_true.p[4]),
                                R_0 = em_true.p[5] + 0.01 * (em_true.p[5]), 
                                C_0 = em_true.p[6] + 0.01 * (em_true.p[6]))
