...
function initialize_params_and_constraints(::PartialObservation, model, data, group_size)
    p_true = model.mp.p
    T = eltype(p_true)
...
function simu(experimental_setup::PartialObservation;
            group_size, noise, adtype, model, batchsize,
            data, epochs)
    ...
    p_init, u0s_init, ranges, distrib_param, p_bij, u0_bij = initialize_params_and_constraints(experimental_setup, model, data, group_size)
