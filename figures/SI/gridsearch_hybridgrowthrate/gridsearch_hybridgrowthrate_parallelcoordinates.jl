...
dict_ticks = Dict(:infer_ics => ["true", "false"], :lr => ["1e-3, 1e-2, 1e-1"], :weight_decay => ["1e-9, 1e-5, 1e-1"],
    :segmentsize => string.(floor.(Int, exp.(range(log(2), log(100), length = 6)))), :HlSize => ["2^2", "2^3", "2^4"])
for j in 1:n_dims
    col = dims[j]
