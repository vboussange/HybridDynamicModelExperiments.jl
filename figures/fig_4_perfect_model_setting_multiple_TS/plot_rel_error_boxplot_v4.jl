...
- v4 : we use data generated on > 2022-06-21,
=#
cd(@__DIR__)
using FileIO, JLD2
...
df_results_allsp = load("../../parameter_inference/3-species-model/inference_exploration/independent_TS/results/2022-06-21/3-species-model_McKann_simple_minibatch_allsp_independent_TS.jld2", "df_results")
df_results_1sp = load("../../parameter_inference/3-species-model/inference_exploration/independent_TS/results/2022-06-21/3-species-model_McKann_simple_minibatch_2sp_independent_TS.jld2", "df_results")
...
idx_noise = [2,3,4] # which noise is considered
#############################
####### plots ###############
#############################
...
for (i,dfg) in enumerate([dfg_noise_allsp,dfg_noise_1sp])
    ...
    for (j,df_noise_i) in enumerate(dfg[idx_noise])
        ...
        for (i,df_results) in enumerate(dfg_nb_ts)
            push!(err_arr, df_results.rel_error); push!(noise_arr, df_results.nb_ts[1])
        end
...
df_forecasting_skill_allsp = load("../../parameter_inference/3-species-model/forecast/results/df_forecasting_skill_3-species-model_McKann_simple_minibatch_allsp_independent_TS.jld2", "df_forecasting_skill")
df_forecasting_skill_1sp = load("../../parameter_inference/3-species-model/forecast/results/df_forecasting_skill_3-species-model_McKann_simple_minibatch_2sp_independent_TS.jld2", "df_forecasting_skill")
idx_timehorizon = 2:4
...
for (i,df) in enumerate([df_forecasting_skill_allsp,df_forecasting_skill_1sp])
    ...
    for (j,df_th_o) in enumerate(dfg)
        global dfg_nb_ts = groupby(df_th_o,"nb_ts", sort = true)
        noise_arr = []
        err_arr = []
        for (i,df_results) in enumerate(dfg_nb_ts)
            push!(err_arr, df_results.forecasting_skill); push!(noise_arr, df_results.nb_ts[1])
        end
