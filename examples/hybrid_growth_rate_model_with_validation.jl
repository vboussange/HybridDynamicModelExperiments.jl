...
ax = Plots.plot()
for tok in tokens(tokenize(dataloader_train))
    segment_data, segment_tsteps = tokenize(dataloader_train)[tok]
    Plots.scatter!(ax, segment_tsteps, segment_data', label = "", c = :blue, alpha = 0.3)
end

for tok in tokens(tokenize(dataloader_valid))
    segment_data, segment_tsteps = tokenize(dataloader_valid)[tok]
    Plots.scatter!(ax, segment_tsteps, segment_data', label = "",
        linestyle = :dash, c = :red, alpha = 0.3)
end
display(ax)
...
res = train(optim_backend,
    lux_model,
    dataloader_train,
    WithValidation(infer_ics, dataloader_valid),
    rng, Lux.f64);
...
plot_segments(dataloader_train, res.ps, st, res.ics)
...
ax = plot_forecast(
    tspan, dataloader_train, res, lux_true_model, data, tsteps, data_with_noise, ps_true, st)
display(ax)
