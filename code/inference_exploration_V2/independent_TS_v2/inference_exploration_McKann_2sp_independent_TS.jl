...
function loss_1sp(data, pred, ic_term)
    l =  mean((data[2:3,:] - pred[2:3,:]).^2)
    l +=  mean((data[2:3,1] - pred[2:3,1]).^2) * ic_term # putting more weights on initial conditions
    return l
end
...
group_size_init = 6+1 # number of points in each segment for multiple shooting (including extra overlapping point)
...
function simu(pars)
    ...
    if noise !== 0.
        fim = sum([FIM_yazdani(em_true, data_sets[i][:,1], (tsteps[i][1], tsteps[i][end]), tsteps[i], em_true.p, Σ) for i in 1:nb_ts]) # TODO: this is probably false, one needs to consider the only availability of predators
    else
        fim = [] # FIM undefined
    end
